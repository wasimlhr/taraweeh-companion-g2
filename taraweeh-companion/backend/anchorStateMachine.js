...
const LOCK_WINS_REQUIRED = 2;     // consecutive chunks must agree when the first hit is weak
  ...
  switch (event.type) {
    ...
    case 'SILENCE':
      if (state.mode === 'LOCKED') {
        const missed = (state.missedChunks || 0) + 1;
        // Callers that tune their miss budget (V4 uses 16) pass it on the
        // event; the module default otherwise made quiet passages drop the
        // lock 4× sooner than the pipeline's own configuration intended.
        const missLimit = event.missBeforeResuming || MISSED_BEFORE_RESUMING;
        if (missed >= missLimit) return { ...state, mode: 'RESUMING', missedChunks: 0 };
