"""
Whisper Quran ONNX inference handler for Modal.
Loads wasimlhr/whisper-quran-v1-onnx-quantized and transcribes WAV audio to Arabic text with optional word-level timestamps.
"""
...
# Model ID on HuggingFace (ONNX dynamic int8 quantized)
MODEL_ID = "wasimlhr/whisper-quran-v1-onnx-quantized"
LANGUAGE = "ar"
TASK = "transcribe"
...
def get_model():
    """Load ONNX model and processor once (reused across requests)."""
...
def transcribe(
    ...
    # No timestamps: use generate directly
    processor, model = get_model()
...
def get_model_card() -> dict:
    ...
    return {
        "model": MODEL_ID,
        "description": "Whisper Large V3 fine-tuned for Quran recitation (ONNX dynamic int8)",
        "language": LANGUAGE,
        "task": TASK,
        "quantization": "onnx-dynamic-int8",
