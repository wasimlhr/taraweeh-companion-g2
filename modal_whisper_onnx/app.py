"""
Modal app: Whisper Quran ONNX — HTTP API for Quran transcription.
...
app = modal.App("whisper-quran-onnx")

# ONNX Runtime GPU, optimum, transformers (no bitsandbytes)
image = (
...
@modal.asgi_app()
def web():
    """Single ASGI app: GET / = model card, POST / = transcribe WAV -> JSON."""
    from fastapi import FastAPI, Request
    from fastapi.responses import JSONResponse

    api = FastAPI(
        title="Whisper Quran ONNX",
        description="Transcribe Quran recitation audio (WAV) to Arabic text. Model: wasimlhr/whisper-quran-v1-onnx-quantized",
    )
